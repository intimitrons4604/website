          ...
          .map((item, index, arr) => {
            return (
              <React.Fragment key={index}>
                ...
                {index < (arr.length - 1) ? <div className="text-box-separator" /> : null}
              </React.Fragment>
            )
          })}
      </Nav>
...
const MediaRow = ({ media, mediaType }) => {
  return media.sort(sortBy("Date"))
    .reverse()
    .map((item, index) => {
      return (
        <React.Fragment key={index}>
          <Row className="mb-5" style={{ 'height': '50px' }} id={getMenuDivId(item.Title)} />
          <Row className="mt-5 mb-5">
            <Col xs={12} sm={12} md={4} lg={4} xl={4} className="pb-5">
              <img src={require(`../images/media-thumbnail/${mediaType}/${item.Thumbnail}`)} width="90%" className="animated fadeIn wow" />
            </Col>
            <Col xs={12} sm={12} md={8} lg={8} xl={8}>
              ...
              <div className="d-flex justify-content-between align-items-end">
                ...
                <div className="p-0 m-0">
                  <h6 className="p-0 m-0 text-right">{moment(item.Date).format("MMMM Do YYYY")}</h6>
                </div>
              </div>
              <p>{item.Snippet}</p>
              <Button href={item.URL} className="trons-green-button trons-small-button">{item.Button}</Button>
            </Col>
          </Row>
        </React.Fragment>
      )
        ...
        <Container fluid={true} className="remove-padding">
          <Row>
            <Col className="remove-padding">
              <div className="trons-hero" style={{ 'backgroundImage': `url(${flickr_37996293252_3f5fcab7d4_h})` }} />
            </Col>
          </Row>
        </Container>
      </Jumbotron>
      ...
      <Container className="mt-5">
        ...
        <Row>
          <Col xs={12} sm={12} md={9} lg={9} xl={9}>
            <script language="javascript" src="https://intimitrons.us3.list-manage.com/generate-js/?u=d479728f08b6043f1d957d5b1&fid=23401&show=10" type="text/javascript" />
          </Col>
          <Col xs={12} sm={12} md={6} lg={3} xl={3} className="pl-xl-5 pl-lg-5 pb-5">
            <ButtonGroup id="trons-vert-team-menu" vertical={true} size={'lg'} aria-label="Trons in the News">
              <Button variant={'secondary'} onClick={() => { window.location.href = 'alienated' }} className="trons-green-button">Alienated</Button>
              <Button variant={'secondary'} onClick={() => { window.location.href = '#articles-menu' }} className="trons-green-button">Articles</Button>
              <Button variant={'secondary'} onClick={() => { window.location.href = '#videos-menu' }} className="trons-green-button">Videos</Button>
            </ButtonGroup>
          </Col>
        </Row>
      </Container>
