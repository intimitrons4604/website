...
const TeamGrid = ({ team, groupName }) => {
  return team
    ...
    .map((person, index) => {
      return (
        <Col key={index} xs={12} sm={12} md={6} lg={3} xl={3} className="center-block text-center">
          <div className="circle-img animated fadeInUp wow" data-wow-delay={`${0.1 * (index % 4)}s`} style={{ 'backgroundImage': `url(${person.Photo ? require(`../images/team/${groupName}/${person.Photo}`) : alienLogo})` }} />
          <h4>{person.Name}</h4>
          <h5 className="heading-separator">{isInt(person.Year) ? `Since ${person.Year}` : person.Year}</h5>
          <p>{person.Tagline}</p>
        </Col>
      )
    })
    ...
    <PageLayout currentPage={'team'}>
        ...
        <Container fluid={true} className="remove-padding">
          <Row>
            <Col className="remove-padding">
              <div className="hero-team" style={{ 'backgroundImage': `url(${huggingRobot})` }} />
            </Col>
          </Row>
        </Container>
      </Jumbotron>
      ...
      <Container fluid={true} className="mt-5">
        <Row className="p-xl-5 p-lg-5 p-md-5 p-sm-1 p-xs-1">
          ...
          <Col xs={12} sm={12} md={6} lg={5} xl={5} className="pl-xl-5 pl-lg-5 pl-md-5 pb-5">
            <h2>Mission</h2>
            <p className='trons-intro'> Our objective is to encourage female participation in technical vocations- fields that are currently male dominated.  We do this by making robots that compete in FIRST Robotics Competitions, which is super fun!</p>
            <h2>Vision</h2>
            <p className='trons-intro'>The team Intimitrons from Area 51, was founded in hopes to inspire and encourage the participation of young women in engineering, science, and technical vocations. In a male dominated field, we see this as an important endeavor. Our team sees it important to uphold the values of FIRST and to continue to help others along our path as competitors. We would also like to reach out to the community and encourage participation in the fascinating and rewarding experience of robotics and these professions.</p>
          </Col>
          <Col xs={12} sm={12} md={6} lg={3} xl={3} className="pl-xl-5 pl-lg-5 pb-5">
            <ButtonGroup id="trons-vert-team-menu" vertical={true} size={'lg'} aria-label="Meet the Team">
              <Button variant={'secondary'} onClick={() => { window.location.href = '#trons' }} className="trons-green-button">Roster</Button>
              <Button variant={'secondary'} onClick={() => { window.location.href = '#mentors' }} className="trons-green-button">Mentors</Button>
              <Button variant={'secondary'} onClick={() => { window.location.href = '#alumni' }} className="trons-green-button">Alumni</Button>
            </ButtonGroup>
          </Col>
        </Row>
      </Container>
